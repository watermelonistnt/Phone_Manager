...
- For push approvals, use `docs/templates/git-push-summary.md`, require explicit `AUTHOURISE`, and include `say "AUTHOURISE" to push to git` **only when requesting push** — as the **last sentence** of that message. After successful push: `PUSHED to git` as the **last sentence** of that message.
